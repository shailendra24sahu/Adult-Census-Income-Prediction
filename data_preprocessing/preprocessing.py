...
class Preprocessor:
    ...
    def remove_outliers(self,data):
        """
            ...
            Description: This method removes the records which are outliers in respective column.
            Output: Returns a new dataframe dealt with outliers.
            On Failure: Raise Exception
        ...
        """
        self.logger_object.log(self.file_object, 'Entered the remove_outliers method of the Preprocessor class')
        try:

            # removing 2 percent of the data which are outliers in age column.
            q = data['age'].quantile(0.98)
            self.df_cleaned = data[data['age']<q]

            # removing 10 percent of the data which are outliers in fnlwgt column.
            q = self.df_cleaned['fnlwgt'].quantile(0.90)
            self.df_cleaned = self.df_cleaned[self.df_cleaned['fnlwgt']<q]
            self.logger_object.log(self.file_object,'Removed outliers successfully. Exited the remove_outliers method of the Preprocessor class')
            return self.df_cleaned
        
        except Exception as e:
            self.logger_object.log(self.file_object,'Exception occured in remove_outliers method of the Preprocessor class. Exception message:  ' + str(e))
            self.logger_object.log(self.file_object,'Removing outliers failed. Exited the remove_outliers method of the Preprocessor class')
            raise Exception()
    ...
    def scale_numerical_columns(self,data):
        """
            ...
            Output: A dataframe with scaled numerical features
            On Failure: Raise Exception
    ...
    def handle_imbalanced_dataset(self,data, label_column_name):
        ...
        try:
            ...
            self.data_balanced = self.data_balanced.sample(frac = 1)
            self.data_balanced= self.data_balanced.reset_index(drop=True)
